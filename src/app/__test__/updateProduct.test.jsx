...
//code login để lấy access_token để call api update product
...
//code test update product with id false
describe.only('Update product with id false',  () => {
  ...
  const body = {
    name: "Laptop MSI Gaming Raider GE77HX 12UHS-229VN i9 12900HX/64GB/2TB/17.3UHD/Nvidia RTX 3080Ti 16GB/Win11 - Update",
...
//code test update product with id true
describe.only('Update product with id true',  () => {
    //khai báo biến id
    const id = "ecbf69be-c1e2-41d7-9733-68d55510755a";
    //khai báo biến body kiểu obj có 7 trường là name, description, images, price, amount, percent_discount, product_category_id, user_id (request body)
  const body = {
    name: "Laptop MSI Gaming Raider GE77HX 12UHS-229VN i9 12900HX/64GB/2TB/17.3UHD/Nvidia RTX 3080Ti 16GB/Win11 - Update",
    description: "Siêu máy tính MSI Gaming Raider GE77HX 12UHS-229VN đích thực là một con quái vật cấu hình với những thông số mạnh nhất có thể đi cùng một thiết kế đẹp khó cưỡng. Hãy cùng xem bộ vi xử lý Intel Core i9 12900HX và card đồ họa RTX 3080 Ti có thể mang đến những trải nghiệm đỉnh cao như thế nào.",
    ...
    //viết code test update product with id true (mong đợi trả ra status 200)
    it("should return 200", async () => {
      ...
      .send(body) //gửi body request lên
      .set('Authorization', `Bearer ${access_token}`); //set header Authorization với giá trị access_token
      expect(response.statusCode).toBe(200); //so sánh http status trả ra 200 => test case pass (id đúng có thể update product thành công)        
    })
});


//code test update product with empty name
describe.only('update product with empty name',  () => {
    //khai báo biến id
    const id = "ecbf69be-c1e2-41d7-9733-68d55510755a";
    //khai báo biến body kiểu obj có 7 trường là name, description, images, price, amount, percent_discount, product_category_id, user_id (request body)
  const body = {
    ...
    //viết code test update product with empty name (mong đợi trả ra status 400)
    it("should return 400", async () => {
        //khai báo biến response nhận giá trị response khi call api update product trả về
      const response = await required(`https://gce.onedev.top/api/v1/e-commerce/products`) //link api update product dùng required của thư viện supertest để call api
      .post("") //phương thức post
      .send(body) //gửi body request lên
      .set('Authorization', `Bearer ${access_token}`); //set header Authorization với giá trị access_token
...
//code test update product success with name not empty and length < 255
describe.only('update product tests success',  () => {
    //khai báo biến id
    const id = "ecbf69be-c1e2-41d7-9733-68d55510755a";
    //khai báo biến body kiểu obj có 7 trường là name, description, images, price, amount, percent_discount, product_category_id, user_id (request body)
  const body = {
    name: "Laptop MSI Gaming Raider GE77HX 12UHS-229VN i9 12900HX/64GB/2TB/17.3UHD/Nvidia RTX 3080Ti 16GB/Win11 - Update success",
    description: "Siêu máy tính MSI Gaming Raider GE77HX 12UHS-229VN đích thực là một con quái vật cấu hình với những thông số mạnh nhất có thể đi cùng một thiết kế đẹp khó cưỡng. Hãy cùng xem bộ vi xử lý Intel Core i9 12900HX và card đồ họa RTX 3080 Ti có thể mang đến những trải nghiệm đỉnh cao như thế nào.",
    images: [
      {
        name: "25",
    ...
    //viết code test update product success (mong đợi kết quả HTTP status 201)
    it("should return 200", async () => {
        //khai báo biến response nhận giá trị response khi call api update product trả về
      const response = await required(`https://gce.onedev.top/api/v1/e-commerce/products`) //link api update product dùng required của thư viện supertest để call api
      .post("") //phương thức post
...
//code test update product success with name is length > 255
describe.only('update product tests success',  () => {
    //khai báo biến id
    const id = "ecbf69be-c1e2-41d7-9733-68d55510755a";
    //khai báo biến body kiểu obj có 7 trường là name, description, images, price, amount, percent_discount, product_category_id, user_id (request body)
  const body = {
    ...
    //viết code test update product with name is length > 255 (mong đợi kết quả HTTP status 400)
    it("should return 400", async () => {
        //khai báo biến response nhận giá trị response khi call api update product trả về
      const response = await required(`https://gce.onedev.top/api/v1/e-commerce/products`) //link api update product dùng required của thư viện supertest để call api
      .post("") //phương thức post
